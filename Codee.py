...
def thermal(button2): #function that controls thermal camera 
    ...
    #range for temperature, corresponds to color spectrum, blue is minimum red is maximum
    MINIMUM_TEMP,MAXIMUM_TEMP = 26.,32. 
    ...
    #make an array storing all colors
    colors_list = [(int(c.red * 255), int(c.green * 255), int(c.INDIGO * 255)) for c in colors_list] 
    ...
    #function that maps value to get resulting color
    def map_value(x, minimum_in, maximum_in, minimum_out, maximum_out): 
...
def face_recognitions(button2): #functions that controls facial_recogniton
    ...
    while True:
        ...
        #resize by 0.25 for faster recognition processing speeds
        singular_frame = cv2.resize(frame, (0, 0), fx=0.25, fy=0.25) 
        ...
        if process:
            ...
            for face_encoding in face_encodings:
                ...
                found_face = face_recognition.compare_faces(recognized_faces, face_encoding)
        ...
        for (top, right, bottom, left), name in zip(recognized_face_locations, recognized_names):
            ...
            #draw a rectangle around face to show
            cv2.rectangle(frame, (left, top), (right, bottom), (0, 0, 255), 2) 

            #make a label under face for name
...
while True: 
    #if the button is pressed and the amount of times it has been pressed is even 
    if button2.is_pressed and button_count%2==0: 
        button_count+=1
        print(button_count)
